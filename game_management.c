...
/*   Updated: 2024/05/24 17:00:28 by yanaranj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_up(t_info *game)
{
	if (game->map[game->y - 1][game->x] != '1')
	{
		...
		if (game->map[game->y][game->x] == 'E' && game->collect == 0)
			exit_win(game);
		if (game->map[game->y][game->x] != 'E')
			mlx_put_image_to_window(game->mlx, game->window, game->bear_up, \
					game->x * game->pos_w, game->y * game->pos_h);
	}
}
...
void	move_down(t_info *game)
{
	if (game->map[game->y + 1][game->x] != '1')
	{
		...
		if (game->map[game->y][game->x] == 'E' && game->collect == 0)
			exit_win(game);
		if (game->map[game->y][game->x] != 'E')
			mlx_put_image_to_window(game->mlx, game->window, game->bear_down, \
					game->x * game->pos_w, game->y * game->pos_h);
	}
}
